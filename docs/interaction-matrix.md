...
| Click a FREQUENT tile TITLE (§4) | recurring activity (kind+title, ≥2 in last 30d) | punch IN: create a same-kind+title occurrence `startAt`=now under the activity's USUAL (modal) parent; DRILLS in | always starts a NEW one (no toggle); band shown by default, toggle §4 / footer "frequent" | [CUR] |
| Click a FREQUENT tile GLYPH | recurring activity with ≤1 ongoing | punch IN + STAY on canvas (start several in a row) | glyph SPINS while any occurrence is ongoing (replaces the old "End…" label) | [CUR] |
| Click a FREQUENT tile GLYPH | recurring activity with >1 ongoing | EXPANDS §4 (same as the counter) so you can end a SPECIFIC instance | with many running, a glyph click is ambiguous → open the list | [CUR] |
| Click a FREQUENT tile (n) COUNTER | activity WITH ≥1 ongoing | EXPAND (animated) §4; ALL tiles reflow into a vertical stack, each showing its OWN list | §4 expand is GLOBAL — every tile's list shows at once; collapsed by default (tiles wrap horizontally) | [CUR] |
...
| In the expanded list: click a row GLYPH | an ongoing occurrence | punch OUT: stamp its `endAt`=now (tz-stable close); stays on canvas; collapses §4 | complete rows have no punch-out | [CUR] |
| In the expanded list: click a row TITLE/meta | an ongoing or complete occurrence | OPEN (drill into) it, no punch-out; collapses §4 | meta = start[–end] · state · duration incl. SECONDS (`1h 35m 00s`) | [CUR] |
| Right-click a FREQUENT tile → log form | recurring activity | form: START time + DURATION (chips/number) + ongoing/ended toggle. "ended"=[start, start+dur] complete; "ongoing"=start, no end. Auto-flips to ended if end lands in the past | stays on canvas; Escape/backdrop dismiss | [CUR] |
| FREQUENT tile dot color | frequent activity | own `accent`, else the bluey Sleep default (`#5566d8`) for sleep-titled, else grey | pure color indicator (glyph carries running state) | [CUR] |
