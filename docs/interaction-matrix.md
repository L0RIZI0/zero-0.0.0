...
| ACTIVITY vs PRESENCE rails on the TODAY lane | scheduled/logged/ongoing occurrence or tracked SESSION vs presence segment | TWO STACKED, TOUCHING RAILS on one band (no gap), laid out in band-PIXELS (`BAND_H=28`): ACTIVITY RAIL (top) = anything with a clock moment — planned (future), logged (past), ONGOING (real past start, no end) — PLUS tracked SESSION spans (`schedule.sessions`, both play + focus); 11px, bottom rests on the seam (`RAIL_SEAM_PX=18`); PRESENCE RAIL (bottom) = recorded presence only (past), 10px, flush at band bottom, NEVER leaves this rail. Presence painted last (stacks in front). Rail contents = `activity` memo (`[...planned, ...sessions]`) | combined lane (`tracks="both"`) only; the standalone ACTIVITY presence lane stays one centered band | [CUR] |
| SESSION bars on the activity rail | any entity in the subtree with `schedule.sessions` (incl. "whenever" playables with no clock time) | one bar per session touching today, clipped to the day: OPEN session (no `endAt`) ⇒ `openEnded` ongoing bar (right edge = now) joining the ongoing stack; CLOSED session ⇒ completed logged span. Both `play` + `focus` kinds shown. This is why a Play'd whenever entity finally appears on the dayline | walks `collectDescendants(ROOT_ID)` (NOT just timed descendants, since playables are un-clocked); tooltip range appends `· play\|focus [· ongoing]` | [CUR] |
| ONGOING bars stack | 2+ entities ongoing at once (each right edge = live now, so they always overlap) — ongoing plans OR open sessions | stacked vertically as lanes within the activity rail, LONGEST duration on TOP; shortest hugs the seam, longer ones lay contiguously upward (`ongoingLanes` memo over `activity`, sorted shortest→longest). Lane height `ONGOING_LANE_H=5`, shrinking toward `ONGOING_LANE_MIN_H=3` when many are ongoing; re-stacks smoothly via the tick's `top` transition as siblings start/stop | a single ongoing bar just rests on the seam; lit/hovered ticks use their own pop height | [CUR] |
...
| Session storage (all entities) | any entity's clock | sessions live ON the entity as `schedule.sessions: {startAt, endAt?, kind}[]` — the last entry lacking `endAt` is the single OPEN session. Scalar `startAt/endAt` are mirrored from the pairs so single-span readers keep working. NOTHING sub-Space is created anymore (the old `Sessions` Space convention is GONE) | `getSessions`/`getOpenSession`/`hasOpenSession` (pure, in kinds.ts); `openSession`/`closeSession`/`toggleSession` (writes, in data.ts) | [CUR] |
...
| Dayline tick vertical align | single-track lane ⇒ ticks centered (top:50%). COMBINED lane ⇒ two TOUCHING rails laid out in band-pixels: presence flush at band bottom (`center=BAND_H−h/2`), normal activity resting its bottom on the seam (`center=RAIL_SEAM_PX−h/2`), ongoing stacked in lanes — all via `-translate-y-1/2` off a numeric `railTop` | [CUR] |
