...
| Right-click a FREQUENT tile → POPOVER | recurring activity | ONE popover: a bulk-action row **End all (n)** / **Close all (n)** on top (same row, both disabled when n==0), then the block log form directly below — no "Log…" step. End all punches every ongoing OUT → complete; Close all force-closes every one now | Escape/backdrop dismiss; bulk ops stay on canvas | [CUR] |
