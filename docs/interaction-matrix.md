...
| Tick opacity (any dayline) | any tick | PRESENCE ticks fully OPAQUE at all times (the solid record of where I was); PLANNED ticks at 0.4 opacity until hovered (→ full). Root presence = solid theme-bg chip (near-black dark / near-white light) + grey hairline | root fill themed; no color-muting | [CUR] |
...
| Click a FREQUENT tile FRAME (whole card, §4) | recurring activity (kind+title, ≥2 in last 30d) | IDLE → punch IN (create a same-kind+title occurrence `startAt`=now under the activity's USUAL parent) and DRILL in. RUNNING → just OPEN the ongoing occurrence (no second start) | whole card is the click target now, not just the title; band shown by default, toggle §4 / footer "frequent" | [CUR] |
