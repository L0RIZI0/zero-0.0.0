...
| §4 default state | any load | EXPANDED by default; a chevron in the header manually expands/collapses (animated height). `starterPins` persists across reload; a running PLAY session survives reload (focus sessions get cleaned — see hydrate row) | replaced the old auto-collapse behaviour | [CUR] |
