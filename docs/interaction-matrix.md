...
| PLANNED vs RECORDED rails on the TODAY dayline (OPTION A, v0.4.0) | declared/planned span vs recorded engagement | TWO TOUCHING RAILS split at the seam (`RAIL_SEAM_PX=14`, `BAND_H=28`), framed PLAN vs REALITY: PLANNED rail (TOP) = DECLARED spans only (`planned` memo — scheduled occurrences past/future incl. ongoing declared spans); RECORDED rail (BOTTOM) = the viewed entity's RECORDED engagements (`sessions` memo — closed + running). PRESENCE is REMOVED from this lane (now only on the standalone ACTIVITY dayline). Viewing ROOT ⇒ recorded rail == presence (presence = root's engagements). Sub-lanes grow AWAY from the seam (planned up, recorded down) | combined lane (`tracks="both"`) only; standalone `tracks="presence"` lane stays one centered band | [CUR] |
...
| LANE PACKING within a rail | 2+ spans overlapping in the same rail | greedy interval packing (`packLanes`): each bar takes the lowest lane whose last bar ended at/before its start; else a new lane opens. TOP packs by START (tiling declared spans share lane 0); BOTTOM packs LONGEST-FIRST (longest engagement hugs the seam) — this SUBSUMES the old ongoing stack (all running engagements share the now-edge ⇒ fully overlap ⇒ one lane each, longest nearest seam). Overflow = (1) SHRINK-TO-FIT: `laneGeom` shrinks lane height toward `LANE_MIN_H=3` as lanes multiply | DEFERRED (later/larger §3 sizes): (2) bounded lanes + "+N" marker, (3) band grows taller with lane count (tie to Face size) | [PART] |
...
| Dayline tick vertical align | single-track lane ⇒ ticks centered (top:50%). COMBINED lane ⇒ two TOUCHING rails split at the seam (`RAIL_SEAM_PX=14` of `BAND_H=28`): each tick centers on its PACKED LANE center (`laneGeom` — planned lanes grow up from the seam, recorded lanes grow down), via `-translate-y-1/2` off a numeric `railTop` | [CUR] |
