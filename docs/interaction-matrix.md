...
| Presence tick BLEED | a combined-lane presence tick with NO planned occurrence overlapping its window | grows UP out of the bottom presence rail into the empty planned rail above — recenters `railTop` 72%→50% + taller height (`PRESENCE_SPAN_HEIGHT_PX` 22 vs 10), sliding via a `top` transition. When any plan overlaps, it stays in its rail so both read clearly. Overlap tested on the shared pct geometry (`presenceSpanKeys` memo). Flag `PRESENCE_BLEED` | not applied to lit/hovered ticks (own heights); standalone presence lane unaffected | [CUR] |
