...
| Click a FREQUENT tile GLYPH | recurring activity | punch IN + STAY on canvas (start several in a row) | glyph SPINS while any occurrence is ongoing (replaces the old "End…" label) | [CUR] |
| Click a FREQUENT tile (n) COUNTER | activity WITH ≥1 ongoing | EXPAND (animated) a vertical list of that activity's ongoing occurrences; click same/other counter switches/collapses | counter shown only when n>0; §4 collapsed by default | [CUR] |
| In the expanded list: click a row GLYPH | an ongoing occurrence | punch OUT: stamp its `endAt`=now (tz-stable close); stays on canvas; collapses §4 | — | [CUR] |
| In the expanded list: click a row TITLE/meta | an ongoing occurrence | OPEN (drill into) it, no punch-out; collapses §4 | meta = start · ongoing · live duration-so-far | [CUR] |
