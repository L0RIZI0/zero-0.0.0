...
| ACTIVITY vs PRESENCE rails on the TODAY lane | scheduled/logged/ongoing occurrence vs presence segment | TWO STACKED, TOUCHING RAILS on one band (no gap), laid out in band-PIXELS (`BAND_H=28`): ACTIVITY RAIL (top) = anything with a concrete clock moment — planned (future), logged (past), ONGOING (real past start, no end) — 11px, bottom rests on the seam (`RAIL_SEAM_PX=18`); PRESENCE RAIL (bottom) = recorded presence only (past), 10px, flush at band bottom, NEVER leaves this rail. Presence painted last (stacks in front) | combined lane (`tracks="both"`) only; the standalone ACTIVITY presence lane stays one centered band | [CUR] |
| ONGOING bars stack | 2+ entities ongoing at once (each right edge = live now, so they always overlap) | stacked vertically as lanes within the activity rail, LONGEST duration on TOP; shortest hugs the seam, longer ones lay contiguously upward (`ongoingLanes` memo, sorted shortest→longest). Lane height `ONGOING_LANE_H=5`, shrinking toward `ONGOING_LANE_MIN_H=3` when many are ongoing; re-stacks smoothly via the tick's `top` transition as siblings start/stop | a single ongoing bar just rests on the seam; lit/hovered ticks use their own pop height | [CUR] |
