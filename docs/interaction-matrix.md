...
| Dayline tick HIGHLIGHT (cross-component) | an entity focused elsewhere on the canvas | every dayline tick whose `id` matches the focused entity grows to 26px + goes fully opaque (animated). Focus source = a HOVERED ENTITY CONTENT row (transient), else the OPEN context (persistent "you are here"). Threaded as `highlightId` into both daylines (TODAY combined + ACTIVITY presence) | root is excluded as the open-context source (would light every root presence tick); hover wins over open-context | [CUR] |
